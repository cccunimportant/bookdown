...
var Perceptron = function() { // 感知器物件
  this.step=function(x, w, theta) { // 步階函數：計算目前權重 w 的情況下，網路的輸出值為 0 或 1
    var result = w[1] * x[1] + w[2] * x[2] - theta; // y=x1*w1+x2*w2-theta
    if (result >= 0) // 如果結果大於零
      return 1;      //   就輸出 1
    else             // 否則
      return 0;      //   就輸出 0
  ...
  this.training=function(truthTable) { // 訓練函數 training(truthTable), 其中 truthTable 是目標真值表
    var theta = 0.5;  // 閥值
    var rate = 0.01; // 學習調整速率
    var w = [ 0, 0, 0 ]; // 權重
    for (var loop=0; loop<100; loop++) { // 總共訓練一百輪
      var eSum = 0.0;
      for (var i=0; i<truthTable.length; i++) { // 每輪對於真值表中的每個輸入輸出配對，都訓練一次。
        var x = [ 0, truthTable[i][0], truthTable[i][1] ]; // 輸入： x
        var yd = truthTable[i][2];       // 期望的輸出 yd
        var y = this.step(x, w, theta);  // 目前的輸出 y
        var e = yd - y;                  // 差距 e = 期望的輸出 yd - 目前的輸出 y
        eSum += e;                       // 計算差距總和
        var dw = [ 0, 0, 0 ];            // 權重調整的幅度 dw
        dw[1] = rate * x[1] * e; w[1] += dw[1]; // w[0] 的調整幅度為 dw[0]
        dw[2] = rate * x[2] * e; w[2] += dw[2]; // w[1] 的調整幅度為 dw[1]
        if (loop % 10 == 0)
          log("loop=%d x=(%d,%d) w=(%d,%d) y=%d yd=%d e=%d", loop, x[1], x[2], w[1], w[2], y, yd, e);
      }
      if (Math.abs(eSum) < 0.01) return w; // 當訓練結果誤差夠小時，就完成訓練了。
    }
